...
- **国债收益率是本简报的一阶高权重分析维度**，与回购融资 / RRP / 信用并列，甚至在融资端平静、无急性信号时，收益率边际变化本身就是总判断的首要依据。形成 `stance` 时必须显式纳入 1Y / 3Y / 10Y 的边际变化，不得因融资端平稳就压低收益率权重；若收益率出现明显边际移动，总判断必须被其拉动。
...
- 1Y / 3Y / 10Y 必须形成一个短债与长债分析框架：1Y=近端政策路径，3Y=中段再定价，10Y=长期折现率锚；若写收益率风险，必须同时说明三者最新值、边际变化以及 10Y-1Y / 10Y-3Y 斜率含义。
- **1Y / 3Y / 10Y 每一个都必须拆成「水平 + 边际变化」两层**：水平看绝对位置（如 10Y 是否仍在 4.5% 高位、1Y 是否高于现金替代），边际变化看相对上一有效观测（`previous`）的 bp 变动及其对融资压力 / 折现率 / 久期资产估值的含义。边际变化必须量化到 bp 并解释方向含义，不得只写「小幅下行」之类空泛表述。
- 10Y Treasury Yield：必须拆成水平和边际两层。`REAL_10Y` 现已代表 10 年期美国国债收益率 level 风险，高水平代表长期估值压力仍在；`REAL_10Y_MOMENTUM` 表示 10 年期国债收益率边际方向，上行才代表压力正在增强，下行代表压力边际缓和。
- **重点分析收益率边际变化**：当 1Y / 3Y / 10Y 任一项较上一期变动超过约 3bp，或三者斜率（10Y-1Y / 10Y-3Y）发生明显移动时，必须在 `key_takeaways` 中专门给出一条「国债收益率边际变化」解读，并在 `stance.score_text` / `one_liner` 中体现其对总判断的拉动；即使无单一指标单独成风险，收益率边际组合也应作为核心 takeaway 候选。
...
```json
{
  ...
  "narrative_blocks": {
    ...
    "treasury_yields": {
      "label": "偏松|中性|偏紧",
      "one_liner": "一句话判断1Y/3Y/10Y收益率组合及其边际方向",
      "analysis": "必须首先量化 1Y/3Y/10Y 各自相对上一有效观测(previous)的边际变化(bp)并解释对折现率/久期资产/成长股/黄金估值的含义，再覆盖 1Y 短债、3Y 中段、10Y 长债以及 10Y-1Y/10Y-3Y 斜率；国债收益率边际变化是本简报一阶高权重信号，分析权重不低于融资端与信用端。"
    },
    "jpy_carry": {
      "label": "偏紧|中性|偏松",
      "one_liner": "一句话判断",
...
- 必须输出 `narrative_blocks.treasury_yields` 对象，专门分析 1Y / 3Y / 10Y 美国国债收益率；不得用 10Y Real Yield/TIPS 替代 10Y Treasury Yield。该对象分析必须以「边际变化(bp vs previous)」开场并量化，国债收益率是本简报一阶高权重维度，权重不低于融资端与信用端。
